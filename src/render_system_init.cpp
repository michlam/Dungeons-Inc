...
// constants //
///////////////
std::string map_path = textures_path("dungeon_background.jpg"); // where is this used? i assume it's for later
...
template <class T>
void RenderSystem::dynamic_bind_vbo_and_ibo(GEOMETRY_BUFFER_ID gid, std::vector<T> vertices, std::vector<uint16_t> indices)
{
	glBindBuffer(GL_ARRAY_BUFFER, vertex_buffers[(uint)gid]);
	glBufferData(GL_ARRAY_BUFFER,
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index_buffers[(uint)gid]);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,
		sizeof(indices[0]) * indices.size(), indices.data(), GL_DYNAMIC_DRAW);
	gl_has_errors();
}
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	constexpr vec3 grey = { 0.1,0.1,0.1 };
	...
	// Assumes max 4 frames per row and max 2 types per column
	const float ANIM_FRAME_WIDTH = 0.25;
	const float ANIM_FRAME_HEIGHT = 0.5;
	...
	//TODO-M4
	//
	std::vector<TexturedVertex> healthbar_vertices(4);
	const float HB_FRAME_WIDTH = 1.0;
	const float HB_FRAME_HEIGHT = 1.f / 7.f;
	//const float HB_FRAME_HEIGHT = 0.5;


	healthbar_vertices[0].position = { -1.f / 2, +1.f / 2, 0.f };
	healthbar_vertices[1].position = { +1.f / 2, +1.f / 2, 0.f };
	healthbar_vertices[2].position = { +1.f / 2, -1.f / 2, 0.f };
	healthbar_vertices[3].position = { -1.f / 2, -1.f / 2, 0.f };
	healthbar_vertices[3].texcoord = { 0, 0 };
	healthbar_vertices[2].texcoord = { HB_FRAME_WIDTH, 0 };
	healthbar_vertices[1].texcoord = { HB_FRAME_WIDTH, HB_FRAME_HEIGHT };
	healthbar_vertices[0].texcoord = { 0, HB_FRAME_HEIGHT };

	const std::vector<uint16_t> healthbar_indices = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::HEALTHBAR, healthbar_vertices, healthbar_indices);
...
// Initialize library, face, and pixel sizes for freetype. Return non-zero as error code. 
int RenderSystem::init_text_fonts() {
	// Initialize library
	FT_Face face;
	FT_Library library;
	...
	if (error) {
		std::cout << "Error occured during library initialization" << std::endl;
	}

	// Load a font face
	error = FT_New_Face(library, font_path("monogram.ttf").c_str(), 0, &face);
	if (error == FT_Err_Unknown_File_Format) {
		std::cout << "Font file could be opened and read, but font format is unsupported" << std::endl;
	}
	else if (error) {
		std::cout << "Font file could not be opened or read, or is broken" << std::endl;
	}

	// Set pixel size (face, width, height)
	FT_Set_Pixel_Sizes(face, 0, 48);  // Set width to 0 lets face calculate width based on height



	glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // disable byte-alignment restriction

	for (unsigned char c = 0; c < 128; c++) { // load 128 ascii glyphs
	...
	// Follow rendering steps from initializeGlGeometryBuffers
	// The position corresponds to the center of the texture.
	std::vector<TexturedVertex> textured_vertices(4);
	textured_vertices[0].position = { -1.f / 2, +1.f / 2, 0.f };
	textured_vertices[1].position = { +1.f / 2, +1.f / 2, 0.f };
	...
	const std::vector<uint16_t> textured_indices = { 0, 3, 1, 1, 3, 2 };
	dynamic_bind_vbo_and_ibo(GEOMETRY_BUFFER_ID::TEXT_BOX, textured_vertices, textured_indices);
	gl_has_errors();
